...
class _ProductListState extends State<ProductList> {
  ...
  void _toggleWishlist(ProductEntity product) {
    context.read<ProductBloc>().add(ToggleWishlistEvent(product.id ?? 0));
  }
  ...
  void _applySorting(String sortBy, String order) {
    ...
    context.read<ProductBloc>().add(
      SortProductsEvent(sortBy: sortBy, order: order),
    );
  }
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          BlocSelector<ProductBloc, ProductState, bool>(
            selector: (state) {
              return state is ProductLoadSuccess && state.searchQuery != null;
            },
      ...
      body: BlocConsumer<ProductBloc, ProductState>(
        ...
        builder: (context, state) {
          if (state is ProductInitial) {
          ...
          } else if (state is ProductLoadSuccess && state.isLoadingMore) {
            return RefreshIndicator(
              ...
              child: _buildProductGrid(
                state.products,
                true,
                <String>{}, // Empty set for wishlist for now
          ...
          } else if (state is ProductLoadSuccess && state.products.isEmpty) {
            return RefreshIndicator(
              onRefresh: () async {
                context.read<ProductBloc>().add(const GetProductsEvent());
              },
          ...
          child: GridView.builder(
            ...
            itemBuilder: (context, index) {
              if (index < products.length) {
                ...
                return ProductCard(
                  product: product,
                  isWishlisted: wishlistedProducts.contains(
                    product.id.toString(),
                  ),
